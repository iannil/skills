...
触发 engineer-job 后，**首要行动**是调用 Workflow 脚本自动执行全部 6 个阶段：
...
> **这是 engineer-job 的主执行路径。** 所有阶段（init → architect → develop → integrate → deploy → report）
...
Workflow({
  script: "skills/engineer-job/run.wf.js",
  args: { requirements: "...", mode: "auto", projectName: "..." }
})
```
...
通过 Workflow 工具自动执行全部 6 个阶段。各阶段子代理在 Workflow 引擎管理下自动推进，
无需手动调度。
...
**关键特性**：
- **标准协议**：Phase 0 写入 `project-metadata.json`，Phase 1 读取并补充。所有阶段通过文件传递结构化数据，不靠自然语言摘要
...
engineer-job（主会话）
  ...
  │   │  读取 CONTEXT.md + project-metadata.json + job.state.json
  │   │
...
3. **标准协议传递** — 子代理之间的状态通过文件传递（`project-metadata.json` → `CONTEXT.md` → `job.state.json`），不通过对话参数传递
...
用于 init-project → engineer-architect → engineer-orchestrator 之间的结构化数据传递。
由 init-project（Phase 0）创建，architect（Phase 1）补充，orchestrator（Phase 2）消费。
